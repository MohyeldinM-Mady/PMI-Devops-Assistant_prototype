...
def _explicit_entity_identifier(question: str):
    ...
    commit = re.search(r"\bcommit\s+([0-9a-f]{7,40})\b", lowered)
    if commit:
        return "commit", _normalize_identifier(commit.group(1))

    pr = re.search(r"\b(?:pr|pull\s+request)\s*#?\s*(\d+)\b", lowered)
    if pr:
        return "pull_request", _normalize_identifier(pr.group(1))

    issue = re.search(r"\bissue\s*#?\s*(\d+)\b", lowered)
    if issue:
        return "issue", _normalize_identifier(issue.group(1))

    doc = re.search(r"\b(?:readme(?:\.md)?|[\w.-]+\.md)\b", question, re.IGNORECASE)
    if doc and any(
        word in lowered
        for word in ("read", "show", "display", "content", "explain", "documentation")
    ):
        return "doc", _normalize_identifier(doc.group(0))
...
def plan_query(question: str, active_reference: dict | None = None) -> dict:
    ...
    if deterministic:
        return deterministic.sanitized(question).model_dump()
